...
def get_route_weekly_performance(tours_df, plans_df, start_date, end_date, route_type='All'):
    """
    Lấy dữ liệu performance của routes theo tuần (tuần này và tuần trước)
    Trả về: DataFrame với columns: route, route_type, week_type, revenue, num_customers, gross_profit, 
            planned_revenue, planned_customers, planned_gross_profit, revenue_completion, 
            customer_completion, profit_completion
    """
    ...
    # Tính tuần này (current week) - dựa vào start_date và end_date
    current_week_start = start_dt
    current_week_end = end_dt
    
    # Tính số ngày trong tuần hiện tại
    days_in_week = (end_dt - start_dt).days + 1
    
    # Tính tuần trước (last week) - cùng số ngày trước đó
    last_week_end = current_week_start - timedelta(days=1)
    last_week_start = last_week_end - timedelta(days=days_in_week - 1)
    
    results = []
    
    # Xử lý tuần này
    current_data = filter_data_by_date(tours_df, current_week_start, current_week_end)
    confirmed_current = filter_confirmed_bookings(current_data)
    
    if not confirmed_current.empty:
        # Group by route
        route_current = confirmed_current.groupby('route').agg({
            'revenue': 'sum',
            'gross_profit': 'sum',
            'num_customers': 'sum'
        }).reset_index()
        
        # Lấy plan cho tuần này
        plan_mask_current = (
            (plans_df['year'] == current_week_start.year) &
            (plans_df['month'] == current_week_start.month)
        )
        period_plans_current = plans_df[plan_mask_current]
        
        route_plan_current = period_plans_current.groupby('route').agg({
            'planned_revenue': 'sum',
            'planned_customers': 'sum',
            'planned_gross_profit': 'sum'
        }).reset_index()
        ...
        # Merge
        route_perf_current = route_current.merge(route_plan_current, on='route', how='left').fillna(0)
        
        # Tính completion rates
        route_perf_current['revenue_completion'] = np.where(
            route_perf_current['planned_revenue'] > 0,
            (route_perf_current['revenue'] / route_perf_current['planned_revenue'] * 100),
            0
        )
        route_perf_current['customer_completion'] = np.where(
            route_perf_current['planned_customers'] > 0,
            (route_perf_current['num_customers'] / route_perf_current['planned_customers'] * 100),
            0
        )
        route_perf_current['profit_completion'] = np.where(
            route_perf_current['planned_gross_profit'] > 0,
            (route_perf_current['gross_profit'] / route_perf_current['planned_gross_profit'] * 100),
            0
        )
        
        # Thêm thông tin route_type và week_type
        route_perf_current['route_type'] = route_perf_current['route'].apply(classify_route_type)
        route_perf_current['week_type'] = 'Tuần này'
        
        results.append(route_perf_current)
    
    # Xử lý tuần trước
    last_data = filter_data_by_date(tours_df, last_week_start, last_week_end)
    confirmed_last = filter_confirmed_bookings(last_data)
    
    if not confirmed_last.empty:
        # Group by route
        route_last = confirmed_last.groupby('route').agg({
            'revenue': 'sum',
            'gross_profit': 'sum',
            'num_customers': 'sum'
        }).reset_index()
        
        # Lấy plan cho tuần trước
        plan_mask_last = (
            (plans_df['year'] == last_week_start.year) &
            (plans_df['month'] == last_week_start.month)
        )
        period_plans_last = plans_df[plan_mask_last]
        
        route_plan_last = period_plans_last.groupby('route').agg({
            'planned_revenue': 'sum',
            'planned_customers': 'sum',
            'planned_gross_profit': 'sum'
        }).reset_index()
        ...
        # Merge
        route_perf_last = route_last.merge(route_plan_last, on='route', how='left').fillna(0)
        
        # Tính completion rates
        route_perf_last['revenue_completion'] = np.where(
            route_perf_last['planned_revenue'] > 0,
            (route_perf_last['revenue'] / route_perf_last['planned_revenue'] * 100),
            0
        )
        route_perf_last['customer_completion'] = np.where(
            route_perf_last['planned_customers'] > 0,
            (route_perf_last['num_customers'] / route_perf_last['planned_customers'] * 100),
            0
        )
        route_perf_last['profit_completion'] = np.where(
            route_perf_last['planned_gross_profit'] > 0,
            (route_perf_last['gross_profit'] / route_perf_last['planned_gross_profit'] * 100),
            0
        )
        
        # Thêm thông tin route_type và week_type
        route_perf_last['route_type'] = route_perf_last['route'].apply(classify_route_type)
        route_perf_last['week_type'] = 'Tuần trước'
        
        results.append(route_perf_last)
    
    if results:
        combined = pd.concat(results, ignore_index=True)
        
        # Filter theo route_type nếu cần
        if route_type != 'All':
            combined = combined[combined['route_type'] == route_type]
        
        return combined
    else:
        return pd.DataFrame(columns=['route', 'route_type', 'week_type', 'revenue', 'num_customers', 
                                     'gross_profit', 'planned_revenue', 'planned_customers', 
                                     'planned_gross_profit', 'revenue_completion', 'customer_completion', 
                                     'profit_completion'])


def create_route_weekly_chart(route_data, metric='revenue', title='', week_type='Tuần này'):
    """
    Tạo horizontal bar chart cho route performance theo tuần
    metric: 'revenue', 'num_customers', 'gross_profit'
    week_type: 'Tuần này' hoặc 'Tuần trước'
    """
    # Filter theo week_type
    data = route_data[route_data['week_type'] == week_type].copy()
    ...
    if metric == 'revenue':
        value_col = 'revenue'
        completion_col = 'revenue_completion'
        label = 'Doanh Thu'
        unit = 'tỷ đồng'
        # Chuyển đổi từ VND sang tỷ đồng
        data['value_display'] = data[value_col] / 1e9
        color = '#00CC96'  # Xanh lá
    elif metric == 'num_customers':
        value_col = 'num_customers'
        completion_col = 'customer_completion'
        label = 'Lượt Khách'
        unit = 'LK'
        data['value_display'] = data[value_col]
        color = '#FFD700'  # Vàng
    else:  # gross_profit
        value_col = 'gross_profit'
        completion_col = 'profit_completion'
        label = 'Lãi Gộp'
        unit = 'tỷ đồng'
        # Chuyển đổi từ VND sang tỷ đồng
        data['value_display'] = data[value_col] / 1e9
        color = '#D3D3D3'  # Xám nhạt
    
    # Sắp xếp theo value giảm dần và lấy top routes
    data = data.sort_values('value_display', ascending=False).head(20)  # Top 20
    
    # Tính % hoàn thành kế hoạch tuần (trung bình có trọng số theo planned)
    if metric == 'revenue':
        total_planned = data['planned_revenue'].sum()
        total_actual = data['revenue'].sum()
    elif metric == 'num_customers':
        total_planned = data['planned_customers'].sum()
        total_actual = data['num_customers'].sum()
    else:  # gross_profit
        total_planned = data['planned_gross_profit'].sum()
        total_actual = data['gross_profit'].sum()
    
    avg_completion = (total_actual / total_planned * 100) if total_planned > 0 else 0
    
    # Sắp xếp lại để hiển thị từ trên xuống (giá trị lớn nhất ở trên)
    data = data.sort_values('value_display', ascending=True)
    ...
    if metric == 'num_customers':
        ...
        text_values = [f"{v:.0f}" for v in data['value_display']]
    
    fig.add_trace(go.Bar(
        x=data['value_display'],
        y=data['route'],
        orientation='h',
        marker_color=color,
        text=text_values,
        textposition='outside',
        hovertemplate='<b>%{y}</b><br>Giá trị: %{text} ' + unit + '<br>Hoàn thành KH: %{customdata:.1f}%<extra></extra>',
        customdata=data[completion_col]
    ))
    
    # Cập nhật layout
    xaxis_title = f"ĐVT: {unit}"
    fig.update_layout(
        title=dict(text=title if title else label, font=dict(size=16)),
    ...
    # Thêm annotation cho % hoàn thành kế hoạch (footer)
    fig.add_annotation(
        text=f"ĐÃ ĐẠT {avg_completion:.0f}% KẾ HOẠCH TUẦN",
...
def load_route_plan_data(sheet_url, period_name='TẾT'):
    """
    ...
    try:
        ...
        for col in df.columns:
            ...
            if 'NHOM TUYEN' in col_upper or 'NHOM TUYẾN' in col_upper:
                ...
                route_col = col
            elif col_upper == 'LK' or 'LƯỢT KHÁCH' in col_upper:
                customers_col = col
        ...
        # Nhóm theo route và route_type để tổng hợp (nếu có nhiều dòng cho cùng một route)
